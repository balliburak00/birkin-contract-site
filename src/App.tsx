...
type ProductSection = "Chair" | "Armchair" | "Dining Table" | "Coffee Table" | "Bar Stool" | "Lounge" | "Ottoman" | "Outdoor";
type CollectionName = "Arc Collection" | "Loop Collection" | "Haven Collection" | "Cocoon Collection" | "Pure Collection" | "Axis Collection";
...
const products: Product[] = [
  {
    ...
    desc: {
      ...
      ar: "كرسي بذراعين منجد بتكوين ناعم وتفاصيل خشبية دافئة، مصمم لمساحات الضيافة الراقية.",
      ru: "Скульптурное мягкое кресло с мягкими округлыми формами и теплыми деревянными деталями, созданное для refined hospitality интерьеров.",
    },
...
const modelWords: Record<Lang, { one: string; many: string }> = {
  ...
  ar: { one: "نموذج", many: "نماذج" },
  ru: { one: "модель", many: "модели" },
};
...
const heroTrustBadges: Record<Lang, string[]> = {
  ...
  ar: ["إنتاج مخصص", "تغليف للتصدير", "عرض حسب المشروع", "فحص اختياري"],
  ru: ["Индивидуальное производство", "Экспортная упаковка", "Проектное предложение", "Опциональная инспекция"],
};

const projectAreas: Record<Lang, [string, string][]> = {
  en: [
  ...
  ar: [
    ["الفنادق واللوبي", "لغة أثاث متناسقة لمناطق اللوبي والغرف واللاونج."],
    ["المطاعم والمقاهي", "نماذج جلوس وطعام متينة للاستخدام التجاري المتكرر."],
    ["الفلل والسكن", "مقاسات وأقمشة وتشطيبات مخصصة للمشاريع الخاصة."],
    ["الخارج والتراس", "توجيهات مواد مناسبة للشمس والرطوبة والتصدير."],
  ],
  ru: [
    ["Отели и лобби", "Единый мебельный язык для лобби, номеров и lounge зон."],
    ["Рестораны и кафе", "Прочные модели для интенсивного коммерческого использования."],
    ["Виллы и резиденции", "Индивидуальные размеры, ткани и отделки для частных проектов."],
    ["Outdoor и террасы", "Материалы под солнце, влажность и экспортные условия."],
  ],
};
...
const content = {
  en: {
    ...
    options: ["Custom dimensions", "Fabric and finish alternatives", "Sample review for selected projects", "Export-suitable packaging", "Optional third-party inspection"],
  },
  tr: {
    ...
    options: ["Özel ölçü", "Kumaş ve yüzey alternatifleri", "Seçili projelerde numune", "İhracata uygun ambalaj", "Opsiyonel kalite kontrol"],
  },
  ar: {} as typeof content["en"],
  ru: {} as typeof content["en"],
};
...
export default function App() {
  const [lang, setLang] = useState<Lang>(() => {
    ...
    return savedLang === "en" || savedLang === "tr" || savedLang === "ar" || savedLang === "ru" ? savedLang : "en";
  });
  ...
  const activeProductSlug = currentPath.startsWith("/products/") ? currentPath.replace("/products/", "").replace("/", "") : "";
  ...
  const activeGalleryImage = selectedGalleryImage && productGallery.includes(selectedGalleryImage) ? selectedGalleryImage : productGallery[0];

  const filteredProducts = selectedSection === "All" ? products : products.filter((product) => product.category === selectedSection);
  ...
  const increaseQty = (name: string) => setCart((current) => current.map((item) => (item.name === name ? { ...item, qty: item.qty + 1 } : item)));
  const decreaseQty = (name: string) => setCart((current) => current.map((item) => (item.name === name ? { ...item, qty: item.qty - 1 } : item)).filter((item) => item.qty > 0));
  ...
  const selectedProductsText = cart.length === 0 ? "-" : cart.map((item) => `- ${item.code} / ${item.name} / ${t.categoryLabels[item.category]} x ${item.qty}`).join("\n");
  ...
  const Header = () => {
    ...
    return (
      <header className={menuOpen ? "navbar editorialNavbar menuOpen" : "navbar editorialNavbar"}>
        ...
        <button type="button" className="menuButton" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)}>
          <span />
          <span />
          <span />
        </button>
        ...
        <nav className={menuOpen ? "mainNav open" : "mainNav"} aria-label="Main navigation">
          {navLinks.map(([label, path]) => (
            <a key={path} href={path} onClick={(event) => { event.preventDefault(); goTo(path); }}>
              {label}
            </a>
          ))}
        </nav>
        ...
        <div className="navActions">
          {(["en", "tr", "ar", "ru"] as Lang[]).map((item) => (
            <button key={item} className={lang === item ? "langBtn active" : "langBtn"} onClick={() => changeLanguage(item)} type="button">
              {item.toUpperCase()}
            </button>
          ))}
    ...
    <>
      <footer className="footer">
        ...
        <div className="footerLinks">
          {[["Products", "/#products"], ["Projects", "/#projects"], ["Materials", "/#materials"], ["Contact", "/#contact"]].map(([label, path]) => (
            <a key={path} href={path} onClick={(event) => { event.preventDefault(); goTo(path); }}>
              {label}
            </a>
          ))}
      ...
      return (
        <main dir={lang === "ar" ? "rtl" : "ltr"}>
          ...
          <section className="section productDetailSection">
            <button type="button" className="secondaryBtn" onClick={() => goTo("/#products")}>{t.backToProducts}</button>
      ...
      <main dir={lang === "ar" ? "rtl" : "ltr"}>
        ...
        <section className="productDetailHero editorialProductHero">
            ...
            {productGallery.length > 1 && (
              <div className="productGalleryThumbs">
                {productGallery.map((image) => (
                  <button type="button" key={image} className={activeGalleryImage === image ? "productGalleryThumb active" : "productGalleryThumb"} onClick={() => setSelectedGalleryImage(image)}>
          ...
          <div className="productDetailContent">
            <button type="button" className="secondaryBtn backProductBtn" onClick={() => goTo("/#products")}>{t.backToProducts}</button>
            <span className="kicker">{activeProduct.collection}</span>
            <div className="productDetailMeta">{activeProduct.code} / {t.categoryLabels[activeProduct.category]}</div>
            ...
            <div className="productDetailOptions">
              ...
              <ul>{t.options.map((item) => <li key={item}>{item}</li>)}</ul>
            </div>
            ...
            <div className="productDetailActions">
              ...
              <button type="button" className="secondaryBtn" onClick={() => goTo("/#quote-list")}>{t.quote}</button>
            </div>
          </div>
        </section>
          ...
          <div className="productCtaBox">
            ...
            <div className="productCtaSide">
              ...
              <div className="productCtaActions">
                <button type="button" className="primaryBtn" onClick={() => addToCart(activeProduct)}>{t.productCtaButton}</button>
                <button type="button" className="secondaryBtn" onClick={() => goTo("/#quote-list")}>{t.quote}</button>
              </div>
            </div>
          </div>
    ...
    <main dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <section className="editorialHero upgradedHero">
        <div className="editorialHeroText">
          ...
          <div className="heroTrustRow">
            {heroTrustBadges[lang].map((badge) => <span key={badge}>{badge}</span>)}
          </div>

          <div className="heroButtons">
            <a href="#collections" className="primaryBtn" onClick={(event) => { event.preventDefault(); goTo("/#collections"); }}>{t.heroPrimary}</a>
            <a href="#quote-list" className="secondaryBtn" onClick={(event) => { event.preventDefault(); goTo("/#quote-list"); }}>{t.heroSecondary}</a>
          </div>
        </div>

        <div className="editorialHeroMedia">
          <span className="editorialHeroLabel">{heroProduct.collection}</span>
          <img key={heroProduct.name} src={heroProduct.image} alt={heroProduct.name} loading="eager" decoding="async" onError={handleImageFallback} />
        </div>
      </section>
      ...
      <section id="collections" className="section collectionShowcase">
        <div className="sectionHead editorialHead">
          ...
          <h2>Collections designed for project coordination.</h2>
          <p>Each collection groups models with a consistent visual language.</p>
        </div>
      ...
      <section id="products" className="section productIndexSection">
        ...
        <div className="typeFilterRow">
          <button type="button" className={selectedSection === "All" ? "typeFilter active" : "typeFilter"} onClick={() => setSelectedSection("All")}>{t.allProducts}</button>
            ...
            <article className="productEditorialCard clickableModelCard" key={product.name} onClick={() => goTo(`/products/${product.slug}`)}>
              ...
              <div className="productEditorialInfo">
                ...
                <div className="modelActions">
                  <button type="button" onClick={(event) => { event.stopPropagation(); goTo(`/products/${product.slug}`); }} className="detailBtn">{t.viewDetails}</button>
